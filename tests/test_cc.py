...
def _compute_adjacency_list_cartesian_ref(indexes, x, y, z, max_distance):
    """
    computes and returns adjacency list
    this is a reference implementation for
    testing purposes only
    """
    adj = []
...
def _compute_cc_ref(indexes, x, y, z, max_distance):
    ...
    adj = _compute_adjacency_matrix_cartesian_ref(indexes, x, y, z, max_distance)
...
def _compute_cc_from_list_ref(indexes, x, y, z, max_distance):
    """
    indexes : list of coordinates (array indexes)
...
def test_adjacency_list_ref(indexes):
    ...
    lst_1 = [
        [[0, 1], [0, 1], [2]],
        [[0], [1]],
        [[0, 1]] + [[i - 1, i, i + 1] for i in range(1, 49)] + [[48, 49]],
    ]

    for i, L in zip(idx, lst_1, strict=False):
        assert _compute_adjacency_list_cartesian_ref(i, x, y, z, 1.0) == L


def test_adjacency_matrix(indexes, dtype):
    x = np.linspace(0, 10, 50, dtype=dtype)
    y = np.linspace(0, 5, 20, dtype=dtype)
    z = np.linspace(0, 1, 5, dtype=dtype)

    assert_array_equal(
        compute_adjacency_matrix_cartesian(indexes, x, y, z, 1.0),
        _compute_adjacency_matrix_cartesian_ref(indexes, x, y, z, 1.0),
    )
...
def test_cc_list_ref(cc_params):
    indexes = [[i, 0, 0] for i in range(3)]
    x = np.array([0, 1, 10])
    y = np.array([0])
    z = np.array([0])
